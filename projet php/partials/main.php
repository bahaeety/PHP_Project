<main>
  <div class="container">
    <div class="d-flex justify-content-between align-items-center">
        <div class="arrow" id="left-arrow">&lt;</div>
        <div class="category-container" id="category-container">
            <div class="category">
                <img src="./images/a-yellow-and-white-background-with-a-logo-that-reads-summer--5099e464865014879c25609f6bdc5f77.avif" alt="Summer SALE">
                <div class="category-title">Offres d'ete</div>
            </div>
            <div class="category">
                <img src="./images/kulturskog-plant-stand-light-green__1185312_pe898352_s5.avif" alt="New">
                <div class="category-title">Nouveautés</div>
            </div>
            <div class="category">
              <img src="./images/furniture-fu001.avif" alt="New">
              <div class="category-title">Tous les meubles</div>
          </div>
            <div class="category">
                <img src="./images/home-improvement-hi001.webp" alt="All furniture">
                <div class="category-title">All furniture</div>
            </div>
            <div class="category">
                <img src="./images/kitchen-appliances-ka001.avif" alt="Summer SALE">
                <div class="category-title">Summer SALE</div>
            </div>
            <div class="category">
                <img src="./images/smart-home-hs001.webp" alt="All furniture">
                <div class="category-title">Maison intelligente</div>
            </div>
            <div class="category">
                <img src="./images/lighting-li001.avif" alt="Summer SALE">
                <div class="category-title">Éclairage</div>
            </div>
            <div class="category">
                <img src="./images/beds-mattresses-bm001.avif" alt="All furniture">
                <div class="category-title">Lits</div>
            </div>
            <div class="category">
                <img src="./images/decoration-de001.avif" alt="Summer SALE">
                <div class="category-title">Décoration intérieure</div>
            </div>
            <div class="category">
                <img src="./images/laundry-cleaning-lc001.avif" alt="New">
                <div class="category-title">Blanchisserie & nettoyage</div>
            </div>
            <div class="category">
                <img src="./images/pet-products-pt001.avif" alt="All furniture">
                <div class="category-title">Produits pour animaux de compagnie</div>
            </div>
            
        </div>
        <div class="arrow" id="right-arrow">&gt;</div>
    </div>
</div>
</main>